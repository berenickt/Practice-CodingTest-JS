...
/*** 우선순위 큐를 구현
 * 데이터의 개수가 𝑁개일때, 구현방식에 따른 시간복잡도는 다음과 같다.
 * 1. 배열 자료형 | 삽입 시간 𝑂(1)    | 삭제 시간 𝑂(𝑁)
 * 2. 힙(Heap)    | 삽입 시간 𝑂(𝑙𝑜𝑔𝑁) | 삭제 시간 𝑂(𝑙𝑜𝑔𝑁)
 *
 * 일반적인 형태의 큐는 선형적인 구조를 가진다
 * 반면에 우선순위 큐는 이진 트리(binary tree) 구조를 사용하는 것이 일반적
 * -- 이진 트리(binary tree) : 최대 2개까지의 자식을 가질 수 있다.
 * -- 포화 이진 트리         : 리프노드를 제외한 모든 노드가 두 자식을 가지고 있는 트리
